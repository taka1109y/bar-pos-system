...
- `orders.js` — Order lifecycle + item add/update/delete; every item mutation runs `recalcTotal()` in a transaction; `GET /open` returns all open orders with charge fields; `POST /` accepts `guest_count`, reads charge settings from `system_settings`, writes `charge_per_person` and `charge_amount` to the order at creation time (`resolveCharge()`/`loadChargeSettings()`). `PATCH /:id/guest-count` updates the guest count and recalculates `charge_amount` (row-locked `FOR UPDATE`, immediate table forced to 0). It keeps the seating-time `charge_per_person` fixed and only recomputes `charge_amount = charge_per_person × new_count` — it does NOT re-resolve the time slot at the current time, so correcting the count after crossing a slot boundary won't change the per-person rate
- `payments.js` — Closes orders (tax-inclusive 内税 calculation); applies late-night surcharge, discount, and gift certificate; resets table status to `available`. Splits items by `tax_category` to calculate standard (10%) and reduced (8%) tax separately; charge/late_night amounts use standard rate; discount applied to standard first then remainder to reduced。**分割会計**: body に任意の `payments`（`[{ method, amount }]`）を渡すと複数支払い方法で登録。各 `amount` の合計は `total` と一致必須（不一致は 400）、金券併用不可（`gift_cert_amount>0` なら 400）。方法別金額を `cash_amount`/`card_amount`/`emoney_amount` に保存し、2方法以上使用時は `payment_method='split'`（1方法のみならその方法名）。単一方法時は選んだ方法のカラム = `total_amount − gift_cert_amount`（**金券は非現金として差し引く**）、他0。不変条件: `cash_amount + card_amount + emoney_amount = total_amount − gift_cert_amount`
- `prices.js` — Returns `current_price` with `pct_change` vs `base_price`; price history
- `reports.js` — 集計系。共通定数 `RECIPE_COST_CTE`（レシピ×材料単価から商品別原価を算出）、`PAID_FILTER`（`void`/`black_cancelled` を除外。`orders o` エイリアス前提）、`RANGE_FILTER`（`$1`=start, `$2`=end, `$3`=TZ）を再利用する
  - `GET /daily?date=YYYY-MM-DD[&since=ISO_TIMESTAMP]`: 日次集計。`since` はレジオープン時刻で、指定時はそれ以降のみ集計。`taxable_standard`/`taxable_reduced`（税率別課税対象額）、`cancel_amount`/`cancel_count`（赤伝票）、`correction_amount`/`correction_count`（黒伝票訂正）を含む。**RegisterClosePage 専用。変更すると閉店処理が壊れる**
  - `GET /analytics?start=&end=`: 売上管理ページ用の期間分析。`summary`（粗利・粗利率・`cost_coverage_pct`・客数・客単価・`avg_stay_minutes`）、`comparison`（`prev_period`＝同日数手前／`prev_week`＝7日手前。単日なら前日・前週同曜日になる。増減率は基準0のとき `null`）、`hourly`、`categories`、`payment_breakdown`、`items` を返す
  - `GET /items?start=&end=`: 商品別の販売数・売上
  - `GET /cost-analysis?start=&end=`: 商品別の原価・粗利・原価率
  - `GET /profit-summary?start=&end=`: 日次の粗利推移
...
- `orders` — id, table_id, status, total_amount, payment_method ('cash'|'card'|'emoney'|'split'), cash_amount, card_amount, emoney_amount (分割会計の方法別金額。単一方法時は選んだ方法=total_amount。合計=total_amount), opened_at, closed_at, guest_count, discount_amount, tax_rate, tax_amount, late_night_rate, late_night_amount, memo, gift_cert_amount, gift_cert_no_change, charge_per_person, charge_amount, receipt_type ('normal'|'black_cancelled'|'void'|'red', default 'normal'), original_order_id (赤伝票・取消し証跡で使用)
...
- **Charge creation & guest-count change**: `POST /api/orders` reads `charge_enabled` and `charge_time_slots` from `system_settings` at order creation time and stores the resolved `charge_per_person` and `charge_amount` on the order row. Changing the guest count later via `PATCH /api/orders/:id/guest-count` recalculates `charge_amount = charge_per_person × new_count` using the **seating-time `charge_per_person`** (it does not re-resolve the time slot at the current time, so a late correction never changes the per-person rate). Charge is otherwise not recalculated (adding/removing items does not touch charge).
...
- **粗利計算**: 定義は `粗利 = SUM(orders.total_amount) − SUM(数量 × レシピ原価)`。`/analytics`・`/cost-analysis`・`/profit-summary` の3本すべてこの定義で、数字は一致する。チャージと深夜料金は原価が無いため丸ごと粗利になる。
  - `RECIPE_COST_CTE` は `LEFT JOIN` のため、**レシピ未登録の商品は原価0＝粗利100%として集計され、粗利が実態より上振れする**。`/analytics` はこれを可視化するため `cost_coverage_pct`（原価設定済み商品の売上構成比）を返し、100%未満なら画面に警告を出す。
  - **売上と原価は必ず別クエリ/別CTEで集計する**。`orders` に `order_items` を JOIN して一度に集計すると `total_amount` が明細行数だけ重複加算される。`fetchRangeTotals()` および `/profit-summary` の `daily_revenue`/`daily_cost` がこの分離パターン。
  - 滞在時間の集計は即会計テーブル（`tables.table_type = 'immediate'`）を除外する（開店と同時に会計されるため平均を0分側に引っ張る）。
...
- **分割会計（複数支払い方法）**: 1会計を現金・カード・電子マネーに金額単位で分割できる。方法別金額を `orders.cash_amount`/`card_amount`/`emoney_amount` に保存し、**不変条件 `cash_amount + card_amount + emoney_amount = total_amount − gift_cert_amount`** を常に保つ（単一方法時は選んだ方法のカラム=`total_amount − gift_cert_amount`）。**金券（商品券）は非現金**として方法別カラムから差し引く（レジクローズの「商品券」行と現金在高の二重計上を防ぐ。釣り有りで金券>総額のときは負値＝払い出す釣り現金を表す）。`payment_method` は代表値（単一なら方法名、2方法以上なら `'split'`）。売上集計・レジ精算の方法別金額は必ず `SUM(cash_amount)` 等（**旧: `SUM(total_amount) FILTER (WHERE payment_method=...)` は使わない**）で集計する。これにより `RegisterClosePage` の現金在高（`cashSystem = registerOpenCash + cashPay.revenue`）が分割の現金分のみを正しく反映する。**金券とは併用不可**（金券を使う会計は単一方法）。UI: `PaymentModal`/`ImmediateCheckoutPanel` の「分割」トグルで方法別金額を入力（割当済=会計総額で会計可）。既存データは `schema.sql` のバックフィルで `payment_method` → 方法別カラムへ移行済み。
...
# POS計装・演出プロジェクト 恒久ルール（pos_work_spec.md より転記・厳守）

以下は `pos_work_spec.md` フェーズ0の指示により恒久ルール化したもの。計装・演出関連の作業では常に厳守する。

## 絶対ルール（厳守）
- 開発中は本番DB・本番環境に触れない。全変更はブランチ上で行う
- 本番デプロイはClaude Codeが実施してよい。ただし下記「デプロイ条件」を全て満たす場合のみ
...
# 価格エンジン（Phase7）凍結パラメータ（変更は要承認）

以下は Phase7 で確定した価格モデルの凍結パラメータ。config は `server/services/pricingModel.js`（BASE_MARKUP・MARKUP_UNIT_TABLE・STEP_RATE・STEP_UNIT・GRID_HALF_SPAN・SEESAW_DIST・COST_FLOOR_MULTIPLIER・PERIOD_MINUTES）と `server/services/rng.js`（シード可能PRNG mulberry32）に集約。**※旧 Phase4/Phase6 の格子関数（snapGrid/softFloor/anchorP6/maxP6/gridStepUp/gridStepDown/hardFloor 等）と soft_floor_08 移行は DEPRECATED（コードは rollback 用に残置・参照しないこと）。band_offset 方式は本モデルで置換・不採用。**

- **運用メモ（market_days）**：取引ナイトは**金曜・土曜固定・週2**。**月曜定休**。**寄り付き（価格リセット）はレジオープンでは自動発火せず、スタッフが手動で任意のタイミングに行う**（価格は前セッションから持ち越す。オーナー指定 2026-08-18）。

- **base** ＝ `menu_items.base_price`（現行実売価格＝定価）。エンジンは書き換えない。**base=0 の時価商品（price_editable）はエンジン・暴落とも対象外**。
- **pricing_base（帯の中心＝オーナーの言う「ベースプライス」・旧 anchor を統合）**＝`round_to_unit(base×1.10)`（`base`＝`menu_items.base_price`＝元の商品価格＝定価。列名は base_price のままだが、価格ゲームの基準＝中心はこの pricing_base）。ここから ±20%・上下計20ステップ（21点）。丸め単位は base で決定：`base<1000→10 ／ 1000≤base<3000→50 ／ base≥3000→100`（round half up）。**markup は engine_on 変動ドリンクの帯中心にのみ効く。engine_off/固定/時価は常に定価＝base（markup非適用）**。
- **呼値 step**＝`max(¥10, floor(pricing_base×0.02 / ¥10)×¥10)`（Phase7R2でオーナー承認 ¥5→**¥10**。2%を¥10単位で切り下げ）。全約定・表示価格は**格子 `pricing_base + n×step`（n∈[−10,+10] の21点）上のみ**。切下げにより帯は原則±20%以内だが、**pricing_base<500（最小¥10がpb×2%を上回る低価格銘柄）のみ±20%を超え得る**（可動域比率は移行レポートで管理）。丸め単位テーブル（`unitForBase`＝pricing_base/crash_floor用）は現行維持（変更なし）。移行は `deploy/migrations/2026-08-21_step_unit_10_*`（可逆・backup表方式。既存 current は新¥10格子へ ROUND スナップ＝持ち越し）。
- **表示基準（板・注文画面の色/▲▼）＝「寄り付き価格(pricing_base=中心)比」**（Phase7R3・オーナー承認。旧「定価(base_price)比」から変更）。サーバの価格ペイロード（`/api/prices`・`prices:sync`・`prices:updated`・暴落items）に `pm.displayInfo()` で `pricing_base`・`n`（中心からの段数）・`center_pct`（寄り付き比%）・`variable`（変動対象か）・`is_crashed` を付与。色規則：**`variable && n>0`＝緑▲／`n<0`＝赤▼／`n=0`＝無色**。**engine_off・ノンアル・時価・フード・ロック品（variable=false）は常に無色**、暴落中は `is_crashed` で **CRASH 表示（赤背景＋残り時間）に上書き**。**base_price（定価）と×1.10 の関係は客側UIに一切出さない**（board「基準値」列は base_price ではなく寄り付き価格 pricing_base を表示、変動幅/%も寄り付き比）。寄り付き直後は全 variable 品 n=0＝全無色。シーソー勝者/犠牲は broadcast の `event`(`seesaw_win`/`seesaw_lose`)＋`delta` で数秒ハイライト（勝者=緑点滅＋▲段/犠牲=赤点滅＋▼段、表示層のみ）。判定は `client/src/utils/priceTone.js` の `priceDisplay()` に集約。凡例「▲▼は本日の寄り付き価格との比較」を各表示付近に常設。
- **価格帯**：`ceiling(=max_price)=pricing_base+10step（≒+20%）／ floor=pricing_base−10step（≒−20%）`。**stored `min_price`＝実効floor＝`max(floor, 原価×1.2格子上スナップ)`**：原価が厳しい銘柄は floor が原価×1.2 で持ち上がる（帯下限が原価割れしない）。移行は `deploy/migrations/2026-08-18_pricing_base_grid_*`（可逆・バックアップ表方式。engine_on 変動ドリンクを新格子へ／engine_off 品は定価固定。暴落中・ロック品・時価は除外）。**2026-08-28: `GRID_HALF_SPAN`（=帯の半幅＝片側の段数）をランタイム可変化（オーナー承認）。既定10（±20%）、`system_settings.grid_half_span` に永続化・起動時 `loadPersistedGridHalfSpan()` で反映。管理画面（システム管理＞プライシング＞動く範囲）で ±20/30/40%（half=10/15/20）を選択→ `PATCH /api/system/settings {grid_half_span}` → `pm.setGridHalfSpan()` 後に `pricingEngine.recomputeBands()` が全 engine_on 変動ドリンクの stored min/max を新スパンで再計算（min=`effectiveFloor(base,cost)`＝原価×1.2床を維持＝原価割れゼロ、max=`ceilingPrice(base)`、current は新[min,max]へクランプ＝広げる時は据置・狭める時のみ丸め・暴落中は据置）＋ `prices:sync` 同期。step（1段の値幅）は不変＝段数のみ増減で格子整合は保たれる。可逆（%を戻せば min/max 復元。狭めて丸めた current のみ非可逆）。マイグレーション不要（値の再計算のみ）。`clampN/priceAtN/nForPrice/floorPrice/ceilingPrice/displayInfo` は `pm.getGridHalfSpan()` を参照。**
- **hard_floor（暴落床）**：Phase7 では実効floor（`max(格子下限 n=−10, 原価×1.2格子)`）と統一していた（旧 base×0.5/0.7 廃止）。**Phase7R で分離・深度復元**（オーナー裁定・9/4興行の目玉演出のため）。暴落床＝`pm.crashFloor(base, cost, ratio)`＝`round_to_unit(max(原価×1.2, pricing_base×ratio))` を**発動時に動的算出**。ratio は `crashSettings.js`：`CRASH_FLOOR_RATIO_DEFAULT=0.5`（engine_on 変動ドリンク＝深く）／`CRASH_FLOOR_RATIO_ENGINE_OFF=0.7`（engine_off かつ暴落可＝高額グラス等・浅め）。**通常下限（実効floor＝stored `min_price`）とは別物**（暴落は pricing_base×比率まで深く落ちる＝`current_price < min_price` が暴落中は常態）。原価が厳しい銘柄は `原価×1.2` で床が持ち上がる。原価欠損時は `pricing_base×ratio` のみ＋発動時警告。
- **運動規則（シーソー・カテゴリ内ゼロサム）**：注文で当該銘柄=勝者が **+k段（抽選 k∈{1,2,3}=0.6/0.3/0.1）** 上昇し、その上昇分を**同カテゴリ（top-level `categories`）の他 engine_on 変動ドリンクへ −1段ずつ抽選配分**（段ベースでゼロサム＝カテゴリの n 合計保存＝平均 pricing_base 維持）。**※犠牲は各銘柄独立の抽選ではなく、勝者の上昇分だけを同カテゴリへ −1段配分する「ゼロサム分配」方式（オーナー承認）**。独立抽選（全 engine_on を毎回抽選で下げる）は平均が下限へ沈み実質値下げになるため不採用。勝者は ceiling、犠牲は各自の実効floor で頭打ち（余地が尽きたら勝者上昇を配分可能分へ縮小）。**時間減衰は廃止**（価格は注文イベントのみで動く。期タイマー15分はカウントダウン表示のみ）。**qty>1 は1注文=1シーソー（v1承認。初回2晩のログで再検討＝バックログ）**、冪等再送は不発。抽選は `rng.js` のシード可能PRNG（テストは env `SEESAW_SEED` 固定で再現／本番は register_opened_at＋連番＋時刻で日々変わる）。event_type=`seesaw_win`/`seesaw_lose`（trigger=`order`）。※呼値が銘柄で異なるため段ゼロサムは円合計を厳密保存しない（差はごく僅か・market_openでリセット）。**2026-08-28: 上昇段 k を +3 超へ拡張可能に（オーナー承認）。`setSeesawDist` は元々 steps≥1 の任意段・sum=1 を受け付ける汎用実装で、UI（システム管理＞プライシング＞上がりやすさ）を +1〜+5段の手入力＋プリセット「もっと派手」（+4/+5を厚め）に拡張。k を増やす＝下がる銘柄が増える（distinct優先1段ずつ＝ゼロサム維持）。勝者の上昇は `up0=min(k, pm.getGridHalfSpan()-nW)` で「動く範囲」の段数が上限（範囲を広げれば k も活きる）。犠牲配分・段ゼロサム・rng は不変。**
- **寄り付き（market open）**：`engine_enabled` のドリンクを **pricing_base（n=0＝ベースプライス）** へ戻す。**レジオープンでは自動発火しない（前セッションの価格を持ち越す）**（オーナー指定 2026-08-18）。実行はスタッフが手動で任意のタイミング（システム管理＞価格モデル＞寄り付きリセット、`POST /api/system/market-open`、trigger='manual'）。event_type=`market_open`。**運用裁定（Phase7R）：金土の営業開始時は価格リセット（寄り付き）必須＝開場儀式。平日は持ち越し容認**。実施状況は `system_settings.last_market_open_at`（`doMarketOpen` が変化有無に関わらず無条件記録＝changed=0でも実施済み検知）に記録。GET `/api/system/settings` が `last_market_open_at`／`market_reset_done`（＝last_market_open_at > register_opened_at のセッション基準）／`is_trading_night`（register_opened_at の JST 曜日が金/土）を返し、管理・レジ画面ヘッダに**「本日の価格リセット未実施」赤バッジ**（金土かつ未実施のみ・socket `market:open` で即消去）を表示。**UI文言は非エンジニア向けに「本日の価格リセット」を主・「寄り付き」は括弧書き**（オーナー指定）。
- **暴落**：`crash_eligible` の銘柄を **crash_floor（Phase7R の暴落床＝上記 hard_floor）へ即時**、5分で**暴落前の格子位置**へ復帰（`price_before`＝発動時に crash_manual へ記録した唯一の真実。復帰は新格子へsnap+[floor,ceiling]クランプ＝stored `[min_price,max_price]`。engine_off 固定品は base へ復帰）。event_type=`crash_manual`/`crash_reset`。**約定は crash_floor 価格で通る**（約定経路 orders.js/payments.js に格子スナップ・アサーションは無く、`current_price` をそのまま `unit_price` へ使う＝暴落中の格子適用除外は自動成立・新規コード不要）。ratio は勝者取得SELECTの `engine_enabled` から解決（`menu.js /crash/manual`）。銘柄単位（カテゴリはスコープ指定のみ）。
- **フラグ（menu_items）**：`engine_enabled`（自動変動）／`crash_eligible`（暴落対象）。初期割当：フード・裏メニュー（客側非表示）・時価・ノンアル・ボトル・薄利（藍茜/萌黄）＝off/off ／ 高額グラス（base≥2000：山崎/山崎12年/macallan）＝off/on ／ 通常アルコール＝on/on。**ノンアル品は常に定価**。商品管理（MenuManager）でトグル編集可（保存経路のみ deprecated `crash_enabled` も同値同期）。**2026-08-18: カテゴリ「ショット＆ワイン」全品を off/off（価格変動・暴落の対象外＝常に定価）へ変更（オーナー指定。`deploy/migrations/2026-08-18_shotwine_no_engine_*`・可逆・backup表方式）。**
...
- **値引き費用（暴落原資）集計**：`Σ max(0, COALESCE(order_items.base_price_at_order, 現行base) − 約定単価) × 数量`（`GET /api/reports/discount-cost`）。約定時 base は `order_items.base_price_at_order` にスナップ（列追加以前は現行base近似）。月次上限＝`system_settings.monthly_discount_cap`（0=無効）、超過で売上管理にアラート。**初期値＝6000円→Phase7R で 25000円へ**（`deploy/migrations/2026-08-16_discount_cap_6000_*`→`2026-08-21_discount_cap_25000_*`・いずれも可逆。深度復元後の9月想定≈8晩×3千円で、6000だと初週で恒常アラート化し計器が死ぬため引上げ）。**Phase7R裁定：値引き基準は旧 `base_price`（約定時スナップ base_price_at_order 優先）で確定・`reports.js` 現行維持**。参考として**純差分 `net_diff = Σ(約定単価 − base) × 数量`（負値可・A1純差分）を同 API（`/discount-cost`）が併記**し、売上管理の値引き費用カードに参考行として表示（値上がりを相殺した実質差）。
