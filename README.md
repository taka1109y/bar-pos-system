...
| URL | 対象 | 説明 |
|-----|------|------|
| `http://localhost/start` | **従業員** | レジオープン前画面。金種入力で開店準備金を登録しレジをオープンする |
| `http://localhost/` | **従業員** | サイドバーナビ付き管理画面（レジオープン中のみアクセス可） |
| `http://localhost/board` | **大型ディスプレイ** | 全ドリンクの現在価格をリアルタイムで表示（TV投影用）。レジオープン前は時計表示 |
| `http://localhost/table/:id` | **お客さん** | セルフ注文・注文確認モーダル（QRコードで配布）。商品画像付き3列グリッド |
| `http://localhost/kitchen` | **キッチン / バー** | 注文アイテム一覧・ステータス管理（提供完了） |

### 管理画面のナビゲーション

左サイドバー（折りたたみ可能）から各機能に切り替える:
...
| レジクローズ | 日計精算レポート・レジ実績入力（金種入力・差異確認）・PDF出力・レジクローズ確定 |
...
| 伝票情報 | 会計済み伝票の詳細一覧（日付フィルター・明細展開・赤伝票発行） |
...
│   │   ├── schema.sql          # PostgreSQL DDL (テーブル + マイグレーション + インデックス)
│   │   └── seed.js             # 初期データ (seed + seedSubcategories)
│   ├── routes/
...
│   │   ├── reports.js          # GET /api/reports/daily (?since=ISO対応)
│   │   ├── receipts.js         # GET /api/receipts  POST /api/receipts/:id/void-and-reissue
│   │   ├── system.js           # GET/PATCH /api/system/settings (register_open含む)
...
│   │   ├── maintenance.js      # POST /api/maintenance/archive (古いデータ削除)
│   │   └── logs.js             # GET /api/logs (会計ログ検索)
│   ├── services/
│   │   ├── pricingEngine.js    # ダイナミック価格エンジン (30秒ティック)
│   │   ├── pricingSettings.js  # 価格エンジンのランタイム設定
│   │   └── socketService.js    # Socket.io io インスタンス共有
│   └── utils/
│       ├── time.js             # TZ・nowInTZ・todayJST・checkLateNight
│       └── validate.js         # assertDateFormat・clampInt
│
└── client/
    ...
    └── src/
        ├── App.jsx             # BrowserRouter + QueryClientProvider + ルートガード
        ...
        ├── pages/
        │   ├── RegisterStartPage.jsx   # レジオープン前画面 (金種入力)
        ...
        │   ├── ReceiptsPage.jsx        # 伝票情報 (赤伝票発行対応)
        │   ├── RegisterClosePage.jsx   # レジクローズ (日計レポート・PDF出力・クローズ確定)
        ...
        └── components/
            ...
            ├── pos/CashDenomModal.jsx      # 金種入力モーダル (レジオープン/クローズ共用)
            ...
            └── menu/PricingSettings.jsx    # 価格エンジンパラメータUI
```
...
order_items     注文明細
                (id, order_id, menu_item_id, quantity, unit_price, item_name,
                 status, created_at)
...
system_settings システム設定 key-value
                (tax_rate, late_night_rate, late_night_start, late_night_end,
                 ...
                 register_open, register_open_cash, register_opened_at)
```
...
| Server→Client | `prices:sync` | `{ items: [...] }` | 接続時の価格フル同期 |
...
| GET | `/api/reports/daily?date=YYYY-MM-DD&since=ISO` | 日次売上集計 (since: レジオープン日時フィルター。void/black_cancelled 除外) |
| GET | `/api/receipts?date=YYYY-MM-DD` | 会計済み伝票一覧 (明細・料金内訳付き。赤伝票含む) |
| POST | `/api/receipts/:orderId/void-and-reissue` | 当日の会計済み伝票を取消し・赤伝票発行 |

### System / Settings / Kitchen / Maintenance / Logs

| Method | Path | 説明 |
|--------|------|------|
| GET | `/api/system/settings` | 全システム設定取得 (register_open 含む) |
| PATCH | `/api/system/settings` | システム設定更新 (tax_rate, reduced_tax_rate, late_night_rate, late_night_start, late_night_end, charge_enabled, charge_time_slots, default_tax_category, register_open, register_open_cash) |
...
| GET | `/api/logs` | 会計ログ検索 (from/to/receipt_type/payment_method/limit/offset) |
| POST | `/api/maintenance/archive` | 古いデータ削除 `{ before_days }` |
...
| `TZ_REPORT` | `Asia/Tokyo` | 日次集計・ログ検索のタイムゾーン |
...
- [x] レジ開閉フロー（開店準備金入力・日計レポート・PDF出力・レジクローズ）
- [x] 赤伝票発行・取消し機能（void-and-reissue）
- [x] 会計ログ検索（receipt_type/payment_method/日付フィルター）
- [x] データアーカイブ（古い会計済みデータの一括削除）
