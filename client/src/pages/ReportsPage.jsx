...
// ─── メインコンポーネント ────────────────────────────
// inline=true: サイドバー管理画面にページとして表示
...
export default function ReportsPage({ onClose, inline = false }) {
  ...
  const content = (
    <div className={inline ? 'p-6 max-w-3xl mx-auto space-y-6' : 'flex-1 overflow-y-auto space-y-6'}>
      {/* 日付選択 */}
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <StatCard
              label="会計件数"
              value={`${report?.order_count || 0}件`}
            />
            <StatCard
              label="平均単価"
              value={`¥${report?.avg_order_value?.toLocaleString() || 0}`}
            />
          </div>
          ...
          {hourlyData?.hourly?.length > 0 && (
            <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
              <h3 className="text-sm font-bold text-gray-700 mb-4">時間別売上</h3>
