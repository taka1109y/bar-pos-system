...
const inp = 'w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500';
...
function SubcategoryRow({ sub, drinkCount, itemCount, onEdit, onDelete }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 bg-white hover:bg-gray-50 group">
      ...
      <div className="flex-1 min-w-0">
        ...
        {isPriceFrozen && (
          <span className="ml-2 px-1.5 py-0.5 text-[10px] font-bold bg-gray-100 text-gray-500 rounded border border-gray-200">
            価格固定
          </span>
        )}
      </div>
...
function InlineForm({ initialValues = {}, fields, onSave, onCancel, isLoading }) {
  ...
  return (
    <form
      ...
      className="flex flex-wrap items-end gap-2 p-3 bg-blue-50 border border-blue-200 rounded-xl"
    >
      {fields.map((f) =>
        f.type === 'select' ? (
      ...
      <div className="flex gap-2 flex-shrink-0">
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="px-3 py-2 text-sm text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors font-bold disabled:opacity-50"
        >
          保存
        </button>
      </div>
...
export default function CategoryManager() {
  ...
  const [addingSubcat, setAddingSubcat] = useState(null); // category_id
  ...
  const createCatMutation = useMutation({
    ...
    onSuccess: () => { invalidate(); setAddingCat(false); },
  });
  ...
  const itemCountBySubcat = menuItems.reduce((acc, item) => {
    if (item.subcategory_id) acc[item.subcategory_id] = (acc[item.subcategory_id] ?? 0) + 1;
    return acc;
  }, {});

  // アクティブなドリンク商品数 (価格固定判定用)
  const drinkCountBySubcat = menuItems.reduce((acc, item) => {
    if (item.subcategory_id && item.is_drink && item.is_active) {
      acc[item.subcategory_id] = (acc[item.subcategory_id] ?? 0) + 1;
    }
    return acc;
  }, {});
  ...
  const catFields = [
    ...
    { key: 'sort_order', label: '表示順序', type: 'number', min: 0, placeholder: '0', className: 'w-24' },
  ];

  const subcatFields = (catId) => [
    {
      key: 'category_id', label: 'カテゴリ', type: 'select', required: true, className: 'w-36',
      options: categories.map((c) => ({ value: c.id, label: c.name })),
    },
    { key: 'name', label: 'サブカテゴリ名', required: true, placeholder: '例: 国産ビール', className: 'flex-1 min-w-36' },
    { key: 'sort_order', label: '表示順序', type: 'number', min: 0, placeholder: '0', className: 'w-24' },
  ];
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {!addingCat && (
          <button
            ...
            className="px-3 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold transition-colors shadow-sm"
          >
            + カテゴリを追加
          </button>
        )}
                ...
                <div className="divide-y divide-gray-100">
                  {subs.map((sub) =>
                    editingSubcat?.id === sub.id ? (
                      ...
                      <SubcategoryRow
                        ...
                        onDelete={(s) => {
                          if (confirm(`「${s.name}」を削除しますか？\n※この商品のサブカテゴリ設定がクリアされます`)) {
                            deleteSubcatMutation.mutate(s.id);
                          }
                        }}
                      />
                    )
                  ...
                  {addingSubcat === cat.id ? (
                    ...
                    <button
                      ...
                      className="w-full px-4 py-2.5 text-left text-xs text-blue-500 hover:bg-blue-50 transition-colors font-medium flex items-center gap-2"
                    >
      ...
      <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-xl">
        <p className="text-xs font-bold text-amber-800 mb-1">価格変動ロジック</p>
        <p className="text-xs text-amber-700 leading-relaxed">
          ...
          商品を追加するか、他のサブカテゴリに移動すると有効になります。
        </p>
      </div>
    </div>
  );
